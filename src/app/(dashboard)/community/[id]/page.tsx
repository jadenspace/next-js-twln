...
export default function PostDetailPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto py-4 md:py-10 px-4 md:px-0">
      <Button
        variant="ghost"
        size="sm"
      ...
      <Card className="mb-6 md:mb-8">
        <CardHeader className="border-b bg-muted/30 px-4 md:px-6 py-3 md:py-4">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] md:text-xs text-muted-foreground mb-1.5 md:mb-2">
            <span className="flex items-center gap-1">
              <User className="w-2.5 h-2.5 md:w-3 md:h-3" /> {post.user?.email}
            </span>
            <span className="flex items-center gap-1">
        ...
        <CardContent className="py-4 md:py-6 px-4 md:px-6 text-xs md:text-sm whitespace-pre-wrap">
          {post.content}
        </CardContent>
      </Card>
      ...
      <div className="space-y-4 md:space-y-6">
        ...
        <div className="space-y-3 md:space-y-4">
          {post.comments?.map((c: any) => (
            <div
              key={c.id}
              className="p-3 md:p-4 rounded-lg bg-muted/50 border"
            >
              ...
              <p className="text-xs md:text-sm">{c.content}</p>
            </div>
          ))}
        </div>
